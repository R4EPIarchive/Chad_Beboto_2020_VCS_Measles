...
# number of hh interivewed from kobo data (= 331)
...
# number of hh interviewed on 1st visit (= # hh visited - hh empty/no adult)
first_visit_interview <- (hh_interviewed-21)
...
##graph of the routine reasons (including OTHER, recoded. column created in chunk 17); this is not weighted. 
routine_all_reasons %>%
  group_by(reason) %>%
  summarise(n = n()) %>%
  ...
  scale_x_discrete(labels = function (reason) str_wrap(reason, width = 7)) # wrapping axis text with long labels
...
##graph of the SIA reasons (including OTHER, recoded. column created in chunk 18)
sia_all_reasons %>%
  group_by(reason) %>%
  summarise(n = n()) %>%
  mutate(prop = n/sum(n)*100) %>%
  ...
  scale_x_discrete(labels = function (reason) str_wrap(reason, width = 8)) # wrapping axis text with long labels
